...
function statusTag(p) {
    ...
    if (p.stopped)
        return '已停手';
    if (state.phase === 'lobby' && p.ready)
        return '已准备';
    if (state.phase === 'evaluation' && p.evaluationDone)
        return '已结算';
...
function chipToken(c, size = 'sm', note = '') {
...
function sceneNarrative(self) {
    if (!self)
        return '<p class="body-copy">当前为观战视角，可从玩家席位加入后参与熬制。</p>';
    const notes = [];
...
function bind() {
    document.querySelectorAll('[data-a]').forEach((el) => el.addEventListener('click', () => {
        const name = el.dataset.a;
        const payload = el.dataset.p ? JSON.parse(el.dataset.p) : undefined;
        act(name, payload);
    }));
    document.querySelectorAll('[data-ui="rules"]').forEach((el) => el.addEventListener('click', () => {
        rulesOpen = true;
        render();
    }));
    document.querySelectorAll('[data-ui="close-rules"]').forEach((el) => el.addEventListener('click', () => {
        rulesOpen = false;
        render();
    }));
}
