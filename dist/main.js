...
const labels = { white: '危险', green: '稳定', orange: '火花', red: '烈焰', blue: '预见' };
const symbols = { white: '✦', green: '☘', orange: '✧', red: '◆', blue: '◈' };
...
function chip(c, size = 'sm') { return `<span class="chip chip-${size} ${c.color}" title="${labels[c.color]} ${c.value}"><i>${symbols[c.color]}</i><b>${c.value}</b></span>`; }
function phaseName(p) { return ({ lobby: '准备室', brewing: '熬制', evaluation: '结算', gameEnd: '终局' })[p]; }
function status(p) { if (!p.connected)
    return '离线'; if (p.exploded)
    return '爆锅'; if (p.pendingBlue)
    return '预见'; if (p.stopped)
    return '停手'; if (state.phase === 'lobby')
    return p.ready ? '已准备' : '等待'; if (state.phase === 'evaluation')
    return p.evaluationDone ? '已结算' : '结算中'; return '进行中'; }
...
function updateBoard(self) {
    ...
    if (!self) {
        node.innerHTML = `<div class="board-empty"><div class="cauldron idle"><div class="liquid"></div><div class="cauldron-label">观战中</div></div><p>加入玩家席位后，这里会显示你的个人药锅。</p></div>`;
        return;
    }
...
function updateCommand(p) {
    ...
    if (!p) {
        node.innerHTML = `<div class="panel-head"><div><span>ACTIONS</span><h2>操作台</h2></div></div><p class="muted">观战状态下没有玩家操作。</p>`;
        return;
    }
...
function commandTitle(p) {
    if (state.phase === 'lobby')
        return '准备开局';
    if (state.phase === 'brewing')
        return p.pendingBlue ? '蓝色预见' : p.exploded ? '锅炉失控' : p.stopped ? '已停手' : '你的回合';
    if (state.phase === 'evaluation')
        return '轮末结算';
    return state.winnerIds.includes(p.id) ? '你赢了！' : '比赛结束';
}
function commandHint(p) {
    if (state.phase === 'brewing' && !p.stopped && !p.exploded)
        return `危险 ${p.danger}/7`;
    if (state.phase === 'evaluation')
        return `${p.coinValue - p.shopSpent} 金币可用`;
    return '';
}
function controls(p) {
    if (state.phase === 'lobby') {
    ...
    if (state.phase === 'brewing') {
        if (p.pendingBlue)
            return `<div class="candidate-list">${p.pendingBlue.map(c => `<button class="candidate" data-a="chooseBlueCandidate" data-p='{"instanceId":"${c.id}"}'>${chip(c, 'lg')}<span><b>${labels[c.color]} ${c.value}</b><small>选择后放入锅中</small></span></button>`).join('')}</div>`;
...
function updateOverlay(self) {
    const node = document.querySelector('#overlay');
    if (rulesOpen) {
        node.innerHTML = rulesPanel();
        return;
    }
    if (detailOpen) {
        node.innerHTML = detailsPanel(self);
        return;
    }
    node.innerHTML = '';
}
function detailsPanel(p) {
    if (!p)
        return modal('局内详情', '<p>观战中暂无个人袋信息。</p>');
    const counts = { white: 0, green: 0, orange: 0, red: 0, blue: 0 };
    p.bag.forEach(c => counts[c.color]++);
    return modal('你的炼金详情', `<div class="detail-grid">${Object.keys(counts).map(c => `<div class="detail-chip">${chip({ id: '', color: c, value: counts[c] }, 'md')}<span>${labels[c]}<b>${counts[c]} 枚</b></span></div>`).join('')}</div><div class="detail-stats"><span>VP <b>${p.vp}</b></span><span>Ruby <b>${p.rubies}</b></span><span>起点 <b>+${p.startBonus}</b></span><span>Flask <b>${p.flaskReady ? '可用' : '已用'}</b></span></div>`);
}
function rulesPanel() {
...
function modal(title, body) { return `<div class="overlay" data-ui="close"><section class="modal panel" onclick="event.stopPropagation()"><div class="modal-head"><h2>${title}</h2><button data-ui="close">×</button></div><div class="modal-scroll">${body}</div></section></div>`; }
function bindUi() {
    document.querySelectorAll('[data-a]').forEach(el => el.onclick = () => act(el.dataset.a, el.dataset.p ? JSON.parse(el.dataset.p) : undefined));
    document.querySelectorAll('[data-ui="rules"]').forEach(el => el.onclick = () => { rulesOpen = true; detailOpen = false; render(); });
    document.querySelectorAll('[data-ui="details"]').forEach(el => el.onclick = () => { detailOpen = true; rulesOpen = false; render(); });
    document.querySelectorAll('[data-ui="close"]').forEach(el => el.onclick = () => { rulesOpen = false; detailOpen = false; render(); });
}
